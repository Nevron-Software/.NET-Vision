...
'			Me.heightScrollbar.ValueChanged += New Nevron.UI.WinForm.Controls.ScrollBarEventHandler(Me.HeightScrollbar_Scroll);
			...
			Me.ShowDesiredSizeCheckBox.TabIndex = 7
...
'			Me.ShowDesiredSizeCheckBox.CheckedChanged += New System.EventHandler(Me.ShowDesiredSizeCheckBox_CheckedChanged);
		...
		Private Sub CreateDiagram()
			Const min As Integer = 100, max As Integer = 200

			Dim i As Integer
			Dim shape As NShape
			Dim random As Random = New Random()
			Dim basicShapesFactory As NBasicShapesFactory = New NBasicShapesFactory(document)

			For i = 0 To 4
				Dim shapeLightColors As Color() = New Color() {Color.FromArgb(236, 97, 49), Color.FromArgb(68, 90, 108), Color.FromArgb(247, 150, 56), Color.FromArgb(129, 133, 133), Color.FromArgb(255, 165, 109)}

				Dim shapeDarkColors As Color() = New Color() {Color.FromArgb(246, 176, 152), Color.FromArgb(162, 173, 182), Color.FromArgb(251, 203, 156), Color.FromArgb(192, 194, 194), Color.FromArgb(255, 210, 182)}

				shape = basicShapesFactory.CreateShape(CInt(Fix(BasicShapes.Rectangle)))
		...
		Private m_Layout As NStackLayout
