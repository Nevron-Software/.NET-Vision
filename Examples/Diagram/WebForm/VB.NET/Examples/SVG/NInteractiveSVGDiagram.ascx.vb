...
'				stateShape.Labels.AddChild(label);
				...
				Dim elementId As String = New NElementIdentifier((CType(stateShape.Primitives.GetChildAt(0), INElement)).Id).ToString()
				Dim stateScript As String = "onmouseover = 'HighlightState(""" & elementId & """, """ & ColorToSVG(stateHighlighColor) & """)' onmouseout = 'HighlightState(""" & elementId & """, """ & ColorToSVG(stateColor) & """)'"
		...
		Protected Function GetScript() As String
			...
			sScript.Append("function Initialize(LoadEvent)" & Constants.vbCrLf)
			sScript.Append("{" & Constants.vbCrLf)
			sScript.Append("SVGDocument = LoadEvent.getTarget().getOwnerDocument()" & Constants.vbCrLf)
			sScript.Append("}" & Constants.vbCrLf)
			sScript.Append(Constants.vbCrLf)
			...
			sScript.Append("{" & Constants.vbCrLf)
				...
				sScript.Append("{" & Constants.vbCrLf)
					...
					sScript.Append("if (new String(element.style) != ""undefined"")" & Constants.vbCrLf)
					sScript.Append("{" & Constants.vbCrLf)
			...
			sScript.Append("function HighlightState(stateID, color)" & Constants.vbCrLf)
			sScript.Append("{" & Constants.vbCrLf)
