<%@ Control Language="vb" Inherits="Nevron.Examples.Diagram.Webform.NCustomCommandsUC"
	CodeFile="NCustomCommandsUC.ascx.vb" %>
...
<table id="exampleRootTable" style="width: 729px; vertical-align: top;" summary="Example layout table">
	<tr>
		<td id="exampleImageCell" class="ImageCell" style="width: 442px; vertical-align: top;">
			...
			<cc1:NDrawingView ID="NDrawingView1" runat="server" Width="416px" Height="320px"
				AjaxEnabled="True" AsyncAutoRefreshEnabled="True" AsyncRefreshInterval="1000"
				OnAsyncRefresh="NDrawingView1_AsyncRefresh" AsyncCallbackTimeout="10000" AsyncRequestWaitCursorEnabled="False"
				OnAsyncCustomCommand="NDrawingView1_AsyncCustomCommand" OnAsyncQueryCommandResult="NDrawingView1_AsyncQueryCommandResult">
			</cc1:NDrawingView>
		...
		<td id="exampleConfigurationCell" class="ControlsPanel" rowspan="2" style="width: 300px;">
			...
			<div id="comment1Paragraph" class="Nested" style="width: 230px; text-align: justify">
				The client side image map is updated on every auto refresh event. Right-click on
				the rotating shapes or the "sun" for a custom context menu.
			</div>
			<label for="autoRefreshCheckbox">
				<input id="autoRefreshCheckbox" name="autoRefreshCheckbox" onclick="UpdateState();"
					type="checkbox" />
				Enable auto-refresh</label><br />
			&nbsp;<br />
			&nbsp;<input id="customCommandButton" onclick="InvokeQueryPositionCustomCommand();"
				style="width: 230px" type="button" value="Invoke Custom Command" /><br />
			&nbsp;<span id="infoSpan"></span><br />
			&nbsp;<br />
			&nbsp;<br />
			<div id="comment2Paragraph" class="Nested" style="width: 230px; text-align: justify">
				The "Boost Rotate" button operation does not refresh the client-side image map.
				Use the "Force Update Image Map" button to get updated image map data.
			</div>
			&nbsp;<input id="rotate10DrgreesButton" onclick="InvokeRotate10DegreesCustomCommand();"
				style="width: 230px" type="button" value="Boost Rotate" /><br />
			&nbsp;<input id="queryXmlImageMapButton" onclick="InvokeQueryXmlImageMapCommand();"
				style="width: 230px" type="button" value="Force Update Image Map" /><br />
...
<!--
	...
	function InvokeQueryPositionCustomCommand()
	{
		if(typeof(NDiagramCallbackService) == "undefined")
			return;
		...
		cs.InvokeCustomCommand("queryPosition", null);
	}
	...
	function CallbackSucceeded(self, result, context)
	{
		switch(context.command)
		{
			case 'queryPosition':
				...
				if(!NReflection.IsInstance(transport.DataSections["position"]))
					break;
				
				var positionText = transport.DataSections["position"].Data;
				var infoSpan = document.getElementById("infoSpan");
				infoSpan.innerHTML = "<i>Current outer orbit body position: " + positionText + "</i>";
				break;
		}
	}
