...
Namespace Nevron.Examples.Diagram.Webform
	...
	''' </summary>
	Public Partial Class NBrowserRedirectUC
		...
		Protected Sub CreateElements(ByVal document As NDrawingDocument)
			Dim fsNonMetals As NFillStyle = New NColorFillStyle(Color.FromArgb(205, 153, 255))
			Dim fsMetals As NFillStyle = New NColorFillStyle(Color.FromArgb(155, 207, 255))
			Dim fsMatalloids As NFillStyle = New NColorFillStyle(Color.FromArgb(48, 198, 187))
			Dim fsGases As NFillStyle = New NColorFillStyle(Color.FromArgb(153, 198, 17))
			Dim feMoccasin As NFillStyle = New NColorFillStyle(Color.FromArgb(255, 204, 157))
			Dim feGold As NFillStyle = New NColorFillStyle(Color.FromArgb(253, 205, 0))
			Dim feCentralGroup As NFillStyle = New NColorFillStyle(Color.FromArgb(255, 154, 204))
		...
		Protected Sub AddElement(ByVal document As NDrawingDocument, ByVal ce As NChemicalElement)
			...
			element.Text = str
			Dim url As String = String.Format("http://www.google.com/search?hl=en&q={0}+chemical+element", HttpUtility.UrlEncode(ce.symbol.Replace("*", String.Empty)))
			element.Style.InteractivityStyle = New NInteractivityStyle(True, Nothing, Nothing, CursorType.Default, url)
