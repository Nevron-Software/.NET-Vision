...
Namespace Nevron.Examples.Chart.WebForm
	Public Partial Class NCustomPaintingUsingGDIPlusUC
		Inherits NExampleUC
		...
		Public Class PaintCallback
			...
			Public Overrides Sub OnAfterPaint(ByVal panel As NPanel, ByVal eventArgs As NPanelPaintEventArgs)
				...
				Dim penRectBorder As Pen = New Pen(Color.DarkBlue)
				Dim brushRectFill As SolidBrush = New SolidBrush(Color.FromArgb(125, Color.LightBlue))

				Dim penPositiveArrowBorder As Pen = New Pen(Color.DarkGreen)
				Dim brushPositiveArrowFill As SolidBrush = New SolidBrush(Color.Green)

				Dim penEqualSignBorder As Pen = New Pen(Color.DarkGray)
				Dim brushEqualSignFill As SolidBrush = New SolidBrush(Color.Gray)

				Dim penNegativeArrowBorder As Pen = New Pen(Color.DarkRed)
				Dim brushNegativeArrowFill As SolidBrush = New SolidBrush(Color.Red)
				...
				Do While i < bar.Values.Count
					...
					Else
						' draw equal sign
						Dim rect As RectangleF = New RectangleF(rcArrowRect.Left, rcArrowRect.Top, rcArrowRect.Width, rcArrowRect.Height / 3.0f)
